...
-- class has exactly one member that serves as its representative element.
-- Every element either is the representative element of its equivalence
-- class or points to another element in the same equivalence class.
-- Equivalence testing thus consists of following the pointers to the
-- representative elements and then comparing these for identity.
--
-- The algorithm performs lazy path compression.  That is, whenever we walk
-- along a path greater than length 1 we automatically update the pointers
-- along the path to directly point to the representative element.
...
-- | /O(1)/. Create a fresh point and return it.  A fresh point is in the
-- equivalence class that contains only itself.
fresh :: a -> IO (Point a)
fresh desc = do
...
-- | /O(1)/. @repr point@ returns the representative point of @point@'s equivalence
-- class.
--
  ...
  case link of
    ...
    Link pt'@(Pt l') -> do
      ...
      when (pt'' /= pt') $ do
        ...
        -- element of @point@'s equivalent class.  Therefore @pt'@'s link
        -- must be of the form @Link r@.  We write this same value into
        -- @point@'s link reference and thereby perform path compression.
        link' <- readIORef l'
        writeIORef l link'
      return pt''
...
-- | /O(1)/. Return the descriptor associated with argument point's equivalence
-- class.
descriptor :: Point a -> IO a
descriptor point = do
  descr <$> (readIORef =<< descrRef point)

-- | /O(1)/. Replace the descriptor of the point's equivalence class with the second
-- argument.
...
-- | /O(1)/. Returns @True@ for all but one element of an equivalence class.  That
-- is, if @ps = [p1, .., pn]@ are all in the same equivalence class, then the
